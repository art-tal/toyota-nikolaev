<template>
    <section>
        <header class="container-fluid">
            <div class="wrap">
                <h1>
                    COROLLA<br>
                    Історія спорткарів Toyota (частина 5)
                </h1>

                <div class="series d-flex justify-content-around">
                    <!--                    <hr/>-->
                    <span>СЕРІЯ HERITAGE</span>
                    <!--                    <hr/>-->
                </div>
            </div>
            <div class="header-image">
                <img src="//d1hu588lul0tna.cloudfront.net/toyotaone/uaua/3-BG_tcm-3046-793762.jpg" alt="photo">
                <img src="//d1hu588lul0tna.cloudfront.net/toyotaone/uaua/2-Copy_tcm-3046-793761.png" alt="toyota">
                <img src="//d1hu588lul0tna.cloudfront.net/toyotaone/uaua/1-Car_tcm-3046-793760.png" alt="cars">
            </div>
        </header>

        <div class="body container">
            <h4>На перший погляд, модель Corolla – найвідоміше сімейне авто від Toyota та справжній світовий бестселер – мала б зовсім недоречний вигляд у компанії справжніх спорткарів. Щоправда, за роки свого існування Corolla випустила кілька дуже популярних спортивних моделей Toyota.</h4>
            <div class="block_right">
                <h5>Corolla демонструє приховані таланти</h5>
                <p>Поза будь-яким сумнівом, найбільш культовою слід вважати Corolla (AE86) Levin середини 1980-х років. На знак пошани фани моделі в усьому світі й понині лагідно називають її AE86 або «Хачі-Року» («вісімдесят шість» японською), за кодом шасі, який використовувала Toyota. На момент виходу моделі решта лінійки Corolla, як і більшість пасажирських автомобілів, переходила на передньоприводну конфігурацію, тоді як популярна серед пуристів компоновка з переднім розташуванням двигуна та заднім приводом була зарезервована для спортивних купе на кшталт Levin.</p>

            </div>

            <img src="//t1-cms-3.images.toyota-europe.com/toyotaone/uaua/2-CorollaAE86-Profile_tcm-3046-793768.jpg" alt="car" class="w-100">
            <img src="//t1-cms-2.images.toyota-europe.com/toyotaone/uaua/3-CorollaAE86-Rear_tcm-3046-793769.jpg" alt="car" class="w-50">
            <img src="//t1-cms-3.images.toyota-europe.com/toyotaone/uaua/4-CorollaAE86-Interior_tcm-3046-793770.jpg" alt="car" class="w-50">

            <div class="block_right">
                <p>Модель AE86 що оснащувалася високообертовим і харизматичним двигуном, об’ємом 1,6 л з двома розподільними валами з верхнім розташуванням (DOHC), що розвивав потужність у 124 «конячки». У поєднанні з динамічним задньоприводним шасі та заниженою посадкою AE86 перетворювалася на справжню мрію водія у її сконцентрованому вираженні. Завдяки цьому автомобіль швидко завоював довіру професійних учасників різноманітних перегонів і ралі, які хотіли отримати грайливу, надійну і швидку задньоприводну модель.</p>
                <p>Героєм фольклору модель AE86 стала завдяки ролі першопрохідця у кіно та у сфері дрифту. Обрана легендарним японським гонщиком Кеїті Цутія (Keiichi Tsuchiya) за надзвичайно збалансовану керованість та контроль, AE86 і досі залишається головним героєм дрифт-шоу та змагань. Але, мабуть, найвідомішою з усіх AE86 слід вважати забарвлену у чорний і білий кольори модель, що з’явилася у класичній манзі Initial D: фільм описував історію Такумі Фудзівара (Takumi Fujiwara), який вдень працював водієм служби доставки, а вночі був королем дрифту. Його улюбленим автомобілем був легендарний Corolla AE86 Trueno. Та й хіба могли бути інші варіанти?</p>
                <h5>Епоха GTi</h5>
                <p>Дещо пізніше у 1980-х роках на сцену виходить інша спортивна модель Corolla, цього разу – Corolla GTi. Протягом цього періоду стандартні сімейні хетчбеки з потужними двигунами, спортивною підвіскою та агресивними комплектами стилізації екстер’єру були надзвичайно популярними, і Corolla GTi з’явилася якраз вчасно.</p>

            </div>

            <img src="//t1-cms-4.images.toyota-europe.com/toyotaone/uaua/5-CorollaAE92-Front_tcm-3046-793778.jpg" alt="car" class="w-100">
            <img src="//t1-cms-4.images.toyota-europe.com/toyotaone/uaua/6-CorollaAE92-Profile_tcm-3046-793779.jpg" alt="car" class="w-50">
            <img src="//t1-cms-4.images.toyota-europe.com/toyotaone/uaua/7-CorollaAE92-Interior_tcm-3046-793780.jpg" alt="car" class="w-50">
            <img src="//t1-cms-2.images.toyota-europe.com/toyotaone/uaua/8-CorollaAE92-Rear_tcm-3046-793781.jpg" alt="car" class="w-100">

            <div class="block_right">
                <p>Автомобіль отримав тюнингований двигун з робочим об’ємом 1587 «кубиків», що був близьким родичем агрегату, яким оснащувався Corolla AE86 Levin. Маючи подвійний розподільний вал з верхнім розташуванням та одноточковою системою впорскування, цей двигун розвивав потужність у 130 к.с. за стандартом DIN і напрочуд жваво реагував на педаль акселератора. У поєднанні з масою, що становила усього 1075 кг, динаміка цього автомобіля була просто неймовірною для свого часу: розгін від 0 до 100 км/год. тривав менше 9 секунд, а максимальна швидкість наближалась до 200 км/год.</p>
                <p>Через збільшену потужність та задля забезпечення активнішої участі водія у керуванні автомобіль отримав удосконалену підвіску, а вентильовані гальма на всіх колесах забезпечують належний рівень безпеки. Спортивний передній бампер і задній спойлер, ефектні бокові обвіси та стильна емблема GTi на задній частині кузова, а також спортивні сидіння з розвиненою боковою підтримкою і триспицеве кермо недвозначно вказували, що цей автомобіль – не просто звичайна Corolla.</p>

            </div>
        </div>

        <div class="container-fluid carousel-img">
            <div class="carousel" @click="show()">
                <swiper class="swiper gallery-thumbs" :options="swiperOptionThumbs" ref="swiperThumbs">
                    <swiper-slide v-for="(img, key) in images" :key="key">
                        <img :src="img" alt="photo">
                        <span><i class="fas fa-search"></i></span>
                    </swiper-slide>


                    <div class="swiper-button-prev" slot="button-prev" @click.stop=""></div>
                    <div class="swiper-button-next" slot="button-next" @click.stop=""></div>
                </swiper>
            </div>

            <div class="carousel_view" v-if="showCarousel">
                <i class="fas fa-times" @click="show()"></i>
                <!--                <swiper class="swiper" :options="swiperOption">-->
                <swiper class="swiper gallery-top" :options="swiperOptionTop" ref="swiperTop">
                    <swiper-slide v-for="(img, key) in images"
                                  :key="key"
                                  :style="'background-image: url(' + img + ')'"></swiper-slide>
                    <div class="swiper-pagination" slot="pagination"></div>
                    <div class="swiper-button-prev" slot="button-prev"></div>
                    <div class="swiper-button-next" slot="button-next"></div>
                </swiper>
            </div>
        </div>

        <continue></continue>

        <div class="next_part container-fluid">
            <img src="//t1-cms-2.images.toyota-europe.com/toyotaone/uaua/9-footer-MR2_tcm-3046-793752.jpg" alt="next car">

            <router-link
                    tag="header"
                    exact
                    to="/world_of_toyota/toyota_mr2"
            >
                <h2>
                    MR2<br>
                    Історія спорткарів Toyota (частина 6)
                </h2>
            </router-link>


        </div>

    </section>
</template>

<script>
    import { Swiper, SwiperSlide } from 'vue-awesome-swiper'
    import 'swiper/css/swiper.css'
    import Continue from "./../../../../components/whyToyota/worldOfToyota/sportCars/Сontinue";

    export default {
        name: "ToyotaCorolla",

        components: {
            Continue,
            Swiper,
            SwiperSlide,
        },

        data() {
            return {

                showCarousel: false,

                images: [
                    "//t1-cms-1.images.toyota-europe.com/toyotaone/uaua/CorollaAE86-Front34-FULL_tcm-3046-795670.jpg",

                    "//t1-cms-1.images.toyota-europe.com/toyotaone/uaua/CorollaAE86-Interior-FULL_tcm-3046-795674.jpg",

                    "//t1-cms-2.images.toyota-europe.com/toyotaone/uaua/CorollaAE86-Profile-FULL_tcm-3046-795678.jpg",

                    "//t1-cms-1.images.toyota-europe.com/toyotaone/uaua/CorollaAE86-Rear34-FULL_tcm-3046-795682.jpg",

                    "//t1-cms-3.images.toyota-europe.com/toyotaone/uaua/CorollaAE92-Front34-FULL_tcm-3046-795686.jpg",

                    "//t1-cms-3.images.toyota-europe.com/toyotaone/uaua/CorollaAE92-Interior-FULL_tcm-3046-795699.jpg",

                    "//t1-cms-4.images.toyota-europe.com/toyotaone/uaua/CorollaAE92-Profile-FULL_tcm-3046-795703.jpg",

                    "//t1-cms-4.images.toyota-europe.com/toyotaone/uaua/CorollaAE92-Rear34-FULL_tcm-3046-795707.jpg",
                ],


                swiperOptionTop: {
                    loop: true,
                    loopedSlides: 5, // looped slides should be the same
                    spaceBetween: 10,
                    navigation: {
                        nextEl: '.swiper-button-next',
                        prevEl: '.swiper-button-prev'
                    }
                },
                swiperOptionThumbs: {
                    loop: true,
                    loopedSlides: 4, // looped slides should be the same
                    spaceBetween: 0,
                    centeredSlides: true,
                    slidesPerView: 4,
                    touchRatio: 0.2,
                    slideToClickedSlide: true,
                    navigation: {
                        nextEl: '.swiper-button-next',
                        prevEl: '.swiper-button-prev'
                    },
                }
            }
        },

        metaInfo() {
        return {
            title:  `Toyota | COROLLA – історія спорткарів Toyota`,
            meta: [
                {
                    vmid: "title",
                    property: "og:title",
                    content: `Toyota | COROLLA – історія спорткарів Toyota`
                },
                {
                    vmid: "description",
                    name: "description",
                    content: `Модель AE86 оснащувалася високообертовим двигуном, об’ємом 1,6 л з двома розподільними валами з верхнім розташуванням (DOHC) потужністю у 124 «конячки».`
                },
                {
                    vmid: "description",
                    property: "og:description",
                    content: `Модель AE86 оснащувалася високообертовим двигуном, об’ємом 1,6 л з двома розподільними валами з верхнім розташуванням (DOHC) потужністю у 124 «конячки».`
                },
                {
                    vmid: "keywords",
                    name: "keywords",
                    content: `Toyota, спорт, спорткари, Corolla, AE86, AE92, Levin, gti, історія, спадщина, heritage, купе, хетчбек, динаміка, дизайн, автоспорт, дрифт, рік, двигун, initial d, manga, Heritage, Corolla Sedan`
                },
                {
                    vmid: "keywords",
                    property: "og:keywords",
                    content: `Toyota, спорт, спорткари, Corolla, AE86, AE92, Levin, gti, історія, спадщина, heritage, купе, хетчбек, динаміка, дизайн, автоспорт, дрифт, рік, двигун, initial d, manga, Heritage, Corolla Sedan`
                },
            ],
        }
    },

        mounted() {
            this.$nextTick(() => {
                const swiperTop = this.$refs.swiperTop.$swiper;
                const swiperThumbs = this.$refs.swiperThumbs.$swiper;
                swiperTop.controller.control = swiperThumbs;
                swiperThumbs.controller.control = swiperTop;
                setTimeout(() => {this.$store.commit("setShowPreload", false);}, 1500)

            })
        },

        methods: {
            show() {
                this.showCarousel = !this.showCarousel;
            }
        },
    }
</script>

<style lang="scss" scoped>
    @import '../../../../styles/variables';

    *{
        margin: 0;
        padding: 0;
    }

    section {
        color: $font_color;
        margin: 0;
        padding: 0;
        header.container-fluid {
            margin: 0;
            padding: 0;
            .wrap {
                background: linear-gradient(to bottom,#CCC 0,#FFF 80%);
                padding: 68px 0;
                h1 {
                    width: 40%;
                    margin: auto;
                    font-size: 3.4rem;
                    line-height: 5.5rem;
                    margin-bottom: 50px;
                    &:first-line {
                        font-size: 4.4rem;
                        font-weight: bolder;
                    }
                }

                .series {
                    position: relative;
                    &::before {
                        content: "";
                        display: block;
                        width: 100px;
                        height: 1px;
                        background: rgba(0,0,0,.2);
                        position: absolute;
                        left: -webkit-calc(50% - 235px);
                        left: -moz-calc(50% - 235px);
                        left: calc(50% - 235px);
                        top: 20px;
                    }
                    &::after {
                        content: "";
                        display: block;
                        width: 100px;
                        height: 1px;
                        background: rgba(0,0,0,.2);
                        position: absolute;
                        right: -webkit-calc(50% - 235px);
                        right: -moz-calc(50% - 235px);
                        right: calc(50% - 235px);
                        top: 20px;
                    }
                    span {
                        display: block;
                        padding: 11px 20px;
                        color: #ffffff;
                        background-color: #c50018;
                        font-size: 1.5rem;
                        letter-spacing: 4px;
                    }

                }
            }
            .header-image {
                position: relative;
                width: 100%;
                img {
                    width: 100%;
                    &:nth-child(2) {
                        position: absolute;
                        bottom: 20px;
                        left: 0;
                    }
                    &:last-child {
                        position: absolute;
                        bottom: 0;
                        left: 0;
                    }
                }
            }
        }

        .body.container {
            margin: 68px auto;
            h4 {
                font-size: 2.2rem;
                margin-bottom: 68px;
            }

            .block_right {
                padding-left: 25%;
                text-align: left;
                margin-bottom: 68px;
                h5 {
                    font-size: 1.8rem;
                    font-weight: bold;
                    margin-bottom: 24px;
                    margin-top: 30px;
                }
                p {
                    font-size: 1.8rem;
                    margin-bottom: 24px;
                    margin-top: 24px;
                }
            }

            img {
                width: 100%;
            }
        }

        .container-fluid.carousel-img {
            /*position: relative;*/
            h2 {
                font-size: 4.4rem;
                margin-bottom: 40px;
                font-weight: bold;
            }

            .swiper.gallery-thumbs {
                height: 200px;
                width: 100%;

                .swiper-slide {
                    display: flex;
                    justify-content: center;
                    align-items: center;
                    text-align: center;
                    font-weight: bold;
                    overflow: hidden;
                    position: relative;
                    /*font-size: $font-size-huge * 2;
                    background-color: $white;*/
                    img {
                        width: 100%;

                    }
                    img + span {
                        position: absolute;
                        top: calc(50% - 24px);
                        left: calc(50% - 24px);
                        display: none;
                        width: 48px;
                        height: 48px;
                        border-radius: 50%;
                        padding: 13px 10px;
                        background-color: rgba(0,0,0,0.5);
                        i {
                            color: #ffffff;
                            font-size: 2rem;
                        }
                    }
                    &:hover {
                        img {
                            transform: scale(1.2);
                            transition-property: all;
                            transition-duration: 1s;
                            cursor: pointer;
                        }
                        img + span {
                            display: block;
                        }
                    }
                }

                .swiper-button-prev {
                    color: #ffffff;
                    left: 50px;
                }
                .swiper-button-next {
                    color: #ffffff;
                    right: 50px;
                }
            }

            .carousel {
                .swiper {
                    height: 200px;
                }
            }

            .carousel_view {
                position: fixed;
                top: 0;
                left: 0;
                width: 100%;
                height: 100%;
                z-index: 998;
                background-color: #fff;
                i {
                    position: absolute;
                    z-index: 999;
                    top: 20px;
                    right: 20px;
                    color: #ffffff;
                    font-size: 3rem;
                }
                .swiper.gallery-top {
                    width: 100%;
                    height: 100%;
                    .swiper-slide {
                        background-repeat: no-repeat;
                        background-position: center center;
                        -webkit-background-size: cover;
                        background-size: cover;
                    }
                }
            }


        }

        .next_part.container-fluid {
            padding: 0;
            position: relative;
            img {
                width: 100%;
            }
            header {
                position: absolute;
                top: 100px;
                left: 0;
                width: 100%;
                color: #ffffff;
                cursor: pointer;
                h2 {
                    font-size: 3.5rem;
                    line-height: 7rem;
                    font-weight: bold;
                    &:first-line {
                        font-size: 5.2rem;
                        font-weight: bolder;
                    }
                }
            }
        }
    }

    @media (min-width: 1200px) and (max-width: 1399px) {
        section {
            header.container-fluid {
                .wrap {
                    h1 {
                        width: 60%;
                    }
                }
            }
        }
    }

    @media (min-width: 992px) and (max-width: 1199.9px) {
        section {
            header.container-fluid {
                .wrap {
                    h1 {
                        width: 80%;
                    }
                }
            }
            .next_part.container-fluid {
                header {
                    position: absolute;
                    top: 50px;
                }
            }
        }
    }

    @media (min-width: 768px) and (max-width: 991.9px) {
        section {
            header.container-fluid {
                .wrap {
                    h1 {
                        width: 100%;
                    }
                }
            }

            .next_part.container-fluid {
                header {
                    position: absolute;
                    top: 30px;
                }
            }
        }
    }

    @media (min-width: 576px) and (max-width: 767.9px) {
        section {
            header.container-fluid {
                .wrap {
                    padding: 40px 0;
                    h1 {
                        width: 100%;
                        font-size: 3rem;
                        line-height: 4.5rem;
                        margin-bottom: 40px;
                        &:first-line {
                            font-size: 3.5rem;
                        }
                    }
                }
            }

            .body.container {
                margin: 30px auto;
                h4 {
                    font-size: 2.0rem;
                    margin-bottom: 10px;
                }

                .block_right {
                    padding: 15px;
                    margin-bottom: 30px;
                    h5 {
                        font-size: 1.8rem;
                    }
                    p {
                        font-size: 1.6rem;
                    }
                }
            }

            .next_part.container-fluid {
                header {
                    top: 30px;
                    h2 {
                        font-size: 2.5rem;
                        line-height: 4rem;
                        &:first-line {
                            font-size: 3.5rem;
                        }
                    }
                }
            }
        }
    }

    @media (max-width: 575.9px) {
        section {
            header.container-fluid {
                .wrap {
                    padding: 40px 0;
                    h1 {
                        width: 100%;
                        font-size: 2rem;
                        line-height: 3.5rem;
                        margin-bottom: 30px;
                        &:first-line {
                            font-size: 3rem;
                        }
                    }

                    .series {
                        &::before {
                            width: 0;
                        }
                        &::after {
                            width: 0;
                        }
                        span {
                            font-size: 1.3rem;
                        }

                    }
                }
            }

            .body.container {
                margin: 30px auto;
                h4 {
                    font-size: 1.8rem;
                    margin-bottom: 0;
                }

                .block_right {
                    padding: 0;
                    margin-bottom: 30px;
                    h5 {
                        font-size: 1.6rem;
                        margin-bottom: 20px;
                        margin-top: 20px;
                    }
                    p {
                        font-size: 1.4rem;
                        margin-bottom: 20px;
                        margin-top: 20px;
                    }
                }
            }

            .next_part.container-fluid {
                header {
                    top: 10px;
                    h2 {
                        font-size: 2.5rem;
                        line-height: 3.5rem;
                        &:first-line {
                            font-size: 3rem;
                        }
                    }
                }
            }
        }
    }

</style>